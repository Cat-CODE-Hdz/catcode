...
export function Pricing() {
  ...
  return (
    <section className="w-full bg-[#F5F4F2] py-unit-20 md:py-unit-24 border-b border-black/[0.06]" id="precios">
      <div className="max-w-[1280px] mx-auto px-gutter-mobile md:px-gutter-desktop">
        ...
        <div className="p-1.5 bg-neutral-200/80 rounded-2xl max-w-fit mb-unit-10 border border-neutral-300/80 shadow-inner flex flex-wrap gap-1">
          {tabs.map((tab) => (
            <button
              key={tab.id}
        ...
        {pricingCategories.map((cat) => {
          const catData = getTabData(cat.id);
          return (
            <div key={cat.id} className={`flex-col ${activeTab === cat.id ? 'flex' : 'hidden'}`}>
              <div className="mb-unit-8 pb-4 border-b border-neutral-300/70 flex flex-col sm:flex-row sm:items-baseline justify-between gap-2">
                <div>
                  <h3 className="font-headline-sm text-headline-sm font-bold text-primary">{catData.title}</h3>
                  <p className="font-body-md text-body-md text-on-surface-variant">{catData.subtitle}</p>
                </div>
                {cat.id === 'custom' && (
              ...
              {cat.id === 'full' ? (
                <div className="max-w-3xl bg-black text-white p-unit-10 rounded-3xl relative shadow-[0_20px_50px_rgba(0,0,0,0.25)] border border-neutral-700">
                  <span className="absolute -top-3.5 right-8 bg-white text-black font-mono uppercase px-3 py-1 rounded-full font-bold tracking-wider shadow-md" style={{ fontSize: '10px' }}>
                    {catData.plans.full.badge}
                  </span>
                  <div>
                    <span className="font-mono text-xs uppercase tracking-widest text-neutral-400 font-semibold">{catData.plans.full.name}</span>
                ...
                <div className={`grid gap-unit-6 ${cat.id === 'social' ? 'grid-cols-1 md:grid-cols-2 max-w-4xl' : 'grid-cols-1 md:grid-cols-3'}`}>
                  {cat.plans.map((plan, planIndex) => {
                    const planData = catData.plans[Object.keys(catData.plans)[planIndex]];
                    return (
                      <div
                        key={plan.name}
                      ...
                      >
                        {planData.badge && (
                          <span className="absolute -top-3 right-6 bg-black text-white font-mono uppercase px-3 py-1 rounded-full font-bold tracking-wider" style={{ fontSize: '10px' }}>
                            {planData.badge}
                          </span>
                        )}
                        <div>
              ...
              {cat.note && (
                <div className="mt-unit-6 p-4 rounded-xl bg-neutral-200/80 border border-neutral-300/70 text-on-surface-variant font-mono text-xs flex items-center gap-2.5">
                  <span className="material-symbols-outlined text-black" style={{ fontSize: '16px' }}>info</span>
